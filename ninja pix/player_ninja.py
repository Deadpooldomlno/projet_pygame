...
class Player():
    ...
    def __init__(self,game):
        ...
        self.img_idleD = [pygame.image.load(f'assets/idle{i}D.png') for i in range(1,3)] #liste avec chaque image de idle orienté vers la droite(quand il fait rien)    
        self.img_idleD = [pygame.transform.scale_by(img, 8) for img in self.img_idleD] #redimensionnement     
        
        self.img_idleG = [pygame.image.load(f'assets/idle{i}G.png') for i in range(1,3)] #meme chose mais orienté vers la gauche
        self.img_idleG = [pygame.transform.scale_by(img, 8) for img in self.img_idleG]  
        
        self.img_runD = [pygame.image.load(f'assets/run{i}D.png') for i in range(1,7)] #liste avec chaque image de run orienté vers la droite(quand il court) 
        self.img_runD = [pygame.transform.scale_by(img, 8) for img in self.img_runD]
        
        self.img_runG = [pygame.image.load(f'assets/run{i}G.png') for i in range(1,7)] #meme chose mais orienté vers la gauche
        self.img_runG = [pygame.transform.scale_by(img, 8) for img in self.img_runG]
        
        self.img_jumpD = pygame.image.load('assets/jumpD.png') #image de jump orienté vers le droite(quand il saute)
        self.img_jumpD = pygame.transform.scale_by(self.img_jumpD, 8)
        
        self.img_jumpG = pygame.image.load('assets/jumpG.png') #meme chose mais orienté vers la gauche
